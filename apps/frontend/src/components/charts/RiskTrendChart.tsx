...
// Line chart showing risk score over past 5-day forecast.
export function RiskTrendChart({ data }: RiskTrendChartProps) {
  if (!data || data.length === 0) {
    return <EmptyState icon={BarChart3} title="No trend data" description="Score history is unavailable." />
  }

  // Deduplicate: keep one entry per date, preferring actual over forecast
  const byDate = new Map<string, SupplierScoreHistory>()
  for (const entry of data) {
    const existing = byDate.get(entry.date)
    if (!existing || (existing.isForecast && !entry.isForecast)) {
